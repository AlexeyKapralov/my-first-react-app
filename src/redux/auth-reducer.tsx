...
export const actions = {
  SetAuthData: (data: object, isAuth: boolean) =>
    ({ type: "auth/SET_DATA_AUTH", data, isAuth } as const),
  SetAuthDataNull: () => ({ type: "auth/SET_DATA_AUTH_NULL" } as const),
  SetErrorsMessages: (messages: object) =>
    ({ type: "auth/SET_ERRORS_MESSAGES", messages } as const),
  setIsInit: (isInit: boolean) =>
    ({ type: "auth/SET_IS_INIT", isInit } as const),
  getCaptcha: (captcha: string | null) =>
    ({ type: "auth/SET_CAPTCHA_SUCCESS", captcha } as const),
};
